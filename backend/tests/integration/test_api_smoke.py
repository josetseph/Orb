...
def test_upload_lands_in_attachments_and_cannot_cross_the_boundary(client, kb):
    name, vault = kb
    r = client.post(
        "/api/v1/upload",
        params={"kb": name, "folder": "Life"},
        files={"file": ("notes.txt", b"hello", "text/plain")},
    )
    assert r.status_code == 200, r.text
    rel = r.json()["rel_path"]
    assert rel.startswith("attachments/Life/notes-") and rel.endswith(".txt")
    assert (vault / rel).read_bytes() == b"hello"
    assert r.json()["url"] == f"/vault-files/{r.json()['url'].split('/')[2]}/{rel}"

    folders = client.get("/api/v1/vault/folders", params={"kb": name}).json()
    assert folders["vault_path"] == str(vault.resolve())
    assert "attachments/Life" in folders["folders"]
    assert rel in {a["rel_path"] for a in folders["attachments"]}

    r = client.post("/api/v1/vault/move", params={"kb": name}, json={"from_rel": rel, "to_rel": "Life/notes.txt"})
