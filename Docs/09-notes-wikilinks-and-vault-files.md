...
**Enrichment blocks.** After multimodal extraction, the ingestion agent places each attachment's extracted text under its link as a delimited block — `<!-- orb:extract src="<link as written>" -->…<!-- /orb:extract -->` (`extract_open(src, mode)`/`EXTRACT_CLOSE`, matched by `EXTRACT_BLOCK_RE`) — and persists it into the `.md`. The opening marker may carry one optional attribute, `<!-- orb:extract src="…" mode="notes" -->`: no mode = graphed in full like the rest of the note; `notes` = the body is a model-written summary followed by the attachment's full text under a `## Transcript` (recordings) or `## Full text` (documents over `LARGE_ATTACHMENT_TOKENS`) heading — the summary is graphed, the full text is indexed as searchable passages. Ingestion writes it on its own; nothing is asked ([11](11-multimedia-enrichment.md) §6.4). The vault move/sweep rewriters match `orb:extract src="…"` up to the closing quote, so the attribute after it does not affect them. The `src` is the link target copied verbatim by `place_extraction`, so `attachment_key` matches block and link even across the legacy/relative URL forms. The bare headers of the pre-marker format are recognised only by `_ENRICHMENT_BLOCK_RE`, which `wrap_legacy_enrichment_blocks` uses to give such blocks markers:
