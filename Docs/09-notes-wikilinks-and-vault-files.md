...
**Enrichment blocks.** After multimodal extraction, the ingestion agent places each attachment's extracted text under its link as a delimited block — `<!-- orb:extract src="<link as written>" -->…<!-- /orb:extract -->` (`extract_open(src, mode)`/`EXTRACT_CLOSE`, matched by `EXTRACT_BLOCK_RE`) — and persists it into the `.md`. The opening marker may carry one optional attribute, `<!-- orb:extract src="…" mode="pending|index|summary" -->`: no mode = graphed like the rest of the note; `pending` = the attachment is over `LARGE_ATTACHMENT_TOKENS` and waits for the user's choice (the block keeps the extracted text); `index` = searchable passages only, nothing graphed; `summary` = the body is a model-written summary, which is what gets graphed ([11](11-multimedia-enrichment.md) §6.4). The vault move/sweep rewriters match `orb:extract src="…"` up to the closing quote, so the attribute after it does not affect them. The `src` is the link target copied verbatim by `place_extraction`, so `attachment_key` matches block and link even across the legacy/relative URL forms. The bare headers of the pre-marker format are recognised only by `_ENRICHMENT_BLOCK_RE`, which `wrap_legacy_enrichment_blocks` uses to give such blocks markers:
