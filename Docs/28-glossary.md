...
| **Enrichment block** | Markdown placed under an attachment link by multimedia processing (transcripts, captions, PDF text), delimited by `<!-- orb:extract src="…" -->…<!-- /orb:extract -->` where `src` is the same vault-relative link as the attachment's; the opening marker may carry `mode="pending|index|summary"` (see Large attachment guard); on re-ingest only blocks whose attachment is gone are removed. Pre-marker blocks get markers once from the vault sweep. | [11](11-multimedia-enrichment.md) |
| **Large attachment guard** | The check in `multimodal_node` that stops a very large attachment from being graphed unasked: extracted text over `LARGE_ATTACHMENT_TOKENS` (default 20000, Models → Local runtime) is parked as a `mode="pending"` enrichment block that keeps the text, the rest of the note is graphed at once, and the note page asks "Graph the whole document" / "Summarize instead" / "Index for search". The answer is stored in `notes.attachment_modes` by `PUT /api/v1/notes/{id}/attachments/mode`, which re-ingests. Images are never parked. | [11](11-multimedia-enrichment.md) |
| **Indexed document** | A large attachment ingested as "Index for search" (`mode="index"` block): one Kuzu node (`kind='indexable'`, `type='document'`, name = lower-cased file name) referenced by the note, its text stored as ~1200-character passages in the isolated-contexts collection and in Meilisearch. No LLM call, no entities: searchable and usable in chat through the matching passages, but absent from multi-hop expansion and the entity panel. | [10](10-ingestion-pipeline.md), [16](16-retrieval-and-chat.md) |
