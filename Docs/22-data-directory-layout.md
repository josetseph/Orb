...
| `notes.attachment_modes` column add | `core/database._sqlite_repairs` | `PRAGMA table_info(notes)` — runs while the column is missing | `ALTER TABLE notes ADD COLUMN attachment_modes JSON` (the user's graph / summary / index answers for large attachments; `create_all` never alters an existing table). |
