...
| `OPENAI_API_KEY` / `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `HUGGINGFACE_API_KEY` | str \| None / `None` | `Settings`; `llm.init_clients`/`_init_ingestion_clients` (raise if missing for the chosen provider), `ai_gate` (HF key only via `provider_is_configured`), `multimedia.py` (OpenAI/Gemini captions) | Cloud credentials. Never persisted anywhere but `.env` | .env only |
...
| `LLM_BASE_URL` | str / `http://127.0.0.1:8080` | `Settings`; `ai_gate` (cloud heuristics), `api/settings.py` (exposed + mutable), `runtime_config` | **Not used to build any client** in current `llm.py`; only influences `ai_is_configured()` and is echoed by the settings API | runtime (`base_url`), .env |
