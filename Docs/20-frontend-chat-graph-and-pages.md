...
- `PROVIDERS` = `local`, `openai`, `gemini`, `anthropic` (Hugging Face is accepted by the backend and the KB panel but not offered here). `LOCAL_PROVIDERS = {local, ollama, lm_studio}` → `isLocal`; for local providers the model fields are hidden (GGUF choice lives in Setup) and an info box explains in-process loading; for cloud providers two text inputs (Chat model, Ingestion model) with per-provider placeholder hints (`CLOUD_MODEL_HINTS`), and a notice that API keys go in `backend/.env` (`OPENAI_API_KEY`, `GOOGLE_API_KEY`, `ANTHROPIC_API_KEY`).
