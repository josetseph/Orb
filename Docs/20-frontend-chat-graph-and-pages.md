...
- `_components/CredentialsPanel.tsx` — one password field per cloud provider. Keys are write-only: the panel shows `configured` and `source` (`keychain` / `from environment`) from `GET /api/v1/credentials`, never the key. Saving calls the Electron bridge (`setCredential`), which encrypts to the OS keychain and pushes to the API; in a browser (no bridge) the fields are disabled and a notice explains why. A red banner appears when the OS reports no keychain, because Orb then refuses to store keys at all.
- `_components/EndpointPanel.tsx` — OpenAI-compatible servers: URL + key, listed by URL. A refresh button calls `GET /api/v1/llm/endpoint-models` and shows the served model names; when the server does not implement `/v1/models` it shows a note telling the user to type the model name instead, rather than blocking.
