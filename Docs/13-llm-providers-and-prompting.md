...
| `openai` | `OPENAI_API_KEY` (raises `ValueError` if missing) | `OpenAI(api_key, timeout=300.0)` | `AsyncOpenAI(timeout=300.0)` | `instructor.patch(OpenAI(timeout=300.0))` (unused in practice) | SDK default `max_retries` (2), 300 s |
...
| `GET /api/v1/kb/{kb_id}/llm` | — | `{kb_id, override:{provider, model, ingestion_model}, effective:{provider, model, ingestion_model, inherited}, providers:[local, openai, gemini, anthropic, huggingface], local_models:[{id, label, size_gb}] (downloaded chat GGUFs only)}`; 404 for unknown non-default KB |
| `PATCH /api/v1/kb/{kb_id}/llm` | `{provider?, model?, ingestion_model?}` — `""`, `null`, `"inherit"`, `"system"`, `"default"` all mean *clear* | Validation: provider ∈ `LLM_PROVIDERS` (400), cloud provider must have its key (`provider_is_configured`, 400 "No API key configured for … — add it to backend/.env first."); if the effective provider is `local`, `model`/`ingestion_model` must be catalog **chat** ids (400 "… is not a known local chat model id.") that are **downloaded** (400 "… is not downloaded — download it in Setup first."). Then `set_llm_config`; then eagerly touches `ctx.llm` so a bad override surfaces now — on exception the override is rolled back to inherit and 400 "Could not initialise that model: …" is returned. Returns the same payload as GET. |

Frontend: `api.getKBLLM(id)`, `api.updateKBLLM(id, {provider, model, ingestion_model})`, types `KBLLMConfig`, `EffectiveLLM`, `KnowledgeBase.llm_*`/`effective_llm`; UI under `frontend/src/app/kb/_components/`.
