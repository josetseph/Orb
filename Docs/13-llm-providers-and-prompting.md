...
- **Storage.** The Electron shell encrypts keys with `safeStorage` into `DATA_DIR/credentials.enc` and pushes them to `PUT /api/v1/credentials`. `CredentialStore` keeps them **in memory only** — `DATA_DIR` is often a synced folder, so a plaintext secret there would leave the machine. `desktop/credentials.js` refuses to store anything when the OS cannot encrypt, rather than degrading to plaintext.
