...
| `frontend/src/components/markdown-editor/htmlToMarkdown.ts` | Rich-text paste → Markdown (turndown + its GFM plugin; atx headings, `-` bullets, fenced code; list padding normalised to one space). Wired in the editor's `paste` handler: files upload, HTML converts, plain text falls through to CodeMirror. HTML with no semantic tags (a code editor's coloured `div`/`span` copy of raw Markdown) returns "" so the plain text is used unescaped. | `htmlToMarkdown` |
...
| `frontend/src/components/note-print-view.tsx` | Export as PDF (header `…` menu): a full react-markdown + GFM + KaTeX rendering of the note portalled into `#print-root`, shown only under `@media print` (globals.css), then `printPage()` — the shell's native print dialog (Save as PDF) or `window.print()` in a browser. Unmounted on `afterprint`. Extraction markers are stripped (their bodies print), wikilinks print as their text, attachments as their names. | `NotePrintView`, `PrintJob` |
...
| `noteId?`, `autoFocus?` | `string`, `boolean` | Per-note memory: on unmount the scroll offset and the serialised editor state (with `historyField`, so ⌘Z still works) are kept in a module map bounded to the last 20 notes; on remount they come back when the note's text is unchanged. `autoFocus` puts the cursor in the body on creation (the page passes it for any non-empty note; an empty note focuses the title). Mount-time work hangs off the wrapper's `onCreateEditor`, since the wrapper creates the view after the host's own effects. |
