...
| `frontend/src/app/notes/_lib/large-attachments.ts` | Finds the blocks ingestion parked as too large: `pendingAttachments(content)` matches `<!-- orb:extract src="…" mode="pending" -->…<!-- /orb:extract -->` and returns `{link, name, tokens}` — `link` = the marker's `src`, `name` = URL-decoded basename with the upload's `-<8 hex>` suffix removed, `tokens` ≈ body chars / 4. Blocks with no mode, `index` or `summary` are ignored. | `pendingAttachments`, `PendingAttachment`, `AttachmentMode` |
| `frontend/src/app/notes/_components/LargeAttachmentPrompt.tsx` | One card per parked attachment asking how to ingest it (§3 table). | `LargeAttachmentPrompt` |
...
│  │                         │  LargeAttachmentPrompt (only for parked attachments) │
│  ├ NotesBatchBar           │  ┌──────────────────────────┬─────────────────────┐  │
...
| `LargeAttachmentPrompt` (under the chips, above the editor) | `pending: PendingAttachment[]; onChoose(link, mode): Promise<void>` | `pending = pendingAttachments(selectedNote.content)` each render; renders nothing when empty. Per attachment: file name, "about Nk tokens · the rest of this note is already in the graph", and three buttons in this order — **"Graph the whole document"** (`graph`, the only `btn-primary`), **"Summarize instead"** (`summary`), **"Index for search"** (`index`), each with a `title` hint. While one request runs every button is disabled and a spinner shows on that card; a failure prints `errMessage(e, "Could not save that choice.")`. `page.tsx` wires `onChoose` to `api.setAttachmentMode(noteId, link, mode, currentKB)` → `ingest.setIngestingNoteIds(prev ∪ {noteId})` (so the status poll picks the note up) → `list.fetchNotes(searchQuery, processedFilter)`. The card disappears when the re-ingest rewrites the block's mode and the body is refreshed. Backend: [11 §6.4](11-multimedia-enrichment.md). |
