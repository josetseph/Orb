...
| `frontend/src/components/markdown-editor/htmlToMarkdown.ts` | Rich-text paste → Markdown (turndown + its GFM plugin; atx headings, `-` bullets, fenced code; list padding normalised to one space). Wired in the editor's `paste` handler: files upload, HTML converts, plain text falls through to CodeMirror. | `htmlToMarkdown` |
