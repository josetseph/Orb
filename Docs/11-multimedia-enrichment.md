...
- **No chunking, no timestamps, English forced.** The feature extractor pads/truncates to a 30-second window, so recordings longer than ~30 s are truncated to their first 30 s unless the installed `transformers` version's `generate` performs long-form sequential decoding for this input shape (it is not requested explicitly — `return_timestamps`/`chunk_length_s` are not set). Treat long recordings as a known limitation.
