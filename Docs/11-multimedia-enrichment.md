...
**What this covers.** How attachments referenced from a note's markdown (PDF, images, audio, video, Word, spreadsheets) are discovered, resolved to files on disk, turned into text — speech by the in-process **Qwen3-ASR** (`multimodal_runtime.py` + `asr_engine.py`, MLX on Apple Silicon or transformers elsewhere, with optional timestamps and speaker labels), video by **Marlin**, images and sparse PDF pages by **the KB's ingestion model itself** (`LLMService.describe_image`: a local GGUF through its vision projector, or a cloud provider's image input), the rest by lightweight parsers (PyMuPDF, python-docx, openpyxl, csv) — and placed into the vault `.md` as delimited enrichment blocks that the extraction stage then reads. It documents `backend/app/services/multimedia.py` end to end, the multimodal phase of the ingestion agent (`multimodal_node`, `parse_attachments`, `classify_attachment`, `extract_attachment`, `place_extraction` in `backend/app/workflows/agents/ingestion_agent.py`), the public entry points of `backend/app/services/multimodal_runtime.py` (`transcribe_audio_path`, `caption_video_path`) with their parameters, the exact block formats and how re-ingest strips them, **notes blocks** (a recording always, and any other non-image attachment over `LARGE_ATTACHMENT_TOKENS`, becomes a model-written summary that is graphed plus its full text kept searchable — automatically, with no prompt, §6.4), ordering and model-residency swaps, temp-file rules, error handling, and every config key involved. Model download internals, the ASR engine choice and GGUF/projector residency belong to [12-local-models-and-inference.md](12-local-models-and-inference.md).
...
| `backend/app/services/asr_engine.py` | Engine choice (`choose`, `detect_engine_for`, `is_asr_bundle`), MLX transcription (`transcribe_with_mlx`), chunking, forced alignment, punctuation restore, pyannote diarization, `[MM:SS] Speaker N:` lines and their untimed form for the summariser | `AsrChoice`, `DEFAULT_REPO`, `ALIGNER_REPO`, `DIARIZER_REPO`, `transcribe_with_mlx`, `restore_punctuation`, `speaker_turns`, `timed_lines`, `untimed` |
...
| `backend/app/api/notes.py` (`_run_attachment_job`) | The editor's per-attachment job; calls the same `finish_attachment` as a full ingest (§6.4) | [07](07-api-reference.md) |
| `backend/app/workflows/ingestion.py` (`_index_documents`, `_passages`) | Indexes the full text of every `mode="notes"` block as searchable passages | [10](10-ingestion-pipeline.md) §6.5 |
| `backend/app/services/llm.py` (`summarize_transcript`, `summarize_document`, `_pieces`) | The summary half of a notes block | [13](13-llm-providers-and-prompting.md) §9.13a–b |
...
- **Segments on Apple Silicon.** `asr_engine.transcribe_with_mlx` takes the decoded mono 16 kHz array, not a path, and transcribes it in pieces of at most `MLX_SEGMENT_SECONDS` (600 s) cut at pauses by `split_audio_into_chunks` — the same bisect-at-the-quietest-window routine the transformers path uses for its 30 s chunks. The library accepts a whole file in one call, but the GPU memory it holds grows with the audio: a 188-minute lecture filled a 24 GB machine and 36 GB of swap. Word times are shifted by each segment's offset and punctuation is restored per segment. The library caches loaded models by path (`_ModelHolder`, LRU of 4), so later segments reuse the weights; Orb clears that cache and the MLX buffer cache when the recording is done, so the speech model is not resident during diarization and the summary.
- **Speaker labels** (`ASR_SPEAKERS`, default on; needs the `aligner` and `diarizer` snapshots — otherwise "Speaker labels skipped" is logged and the plain, unstamped transcript is returned): `asr_engine.speaker_turns` runs pyannote `speaker-diarization-community-1` on the accelerator when there is one (`resolve_torch_device`: `mps`, `cuda`, else `cpu`; 40 s on `mps` against 284 s on `cpu` for a 10-minute slice, identical turns) over the already-decoded waveform with segmentation step `ASR_DIARIZE_STEP` (2.0 s; on CPU 1.0 s is 1.8× realtime and 2.0 s 3.4×, while agreeing on 95.7 % of speech; 3.0 s merges speakers). The pipeline is dropped and the `mps`/`cuda` cache emptied afterwards (`release_accelerator_memory`); on the transformers path the speech model and aligner are unloaded first when the device is not `cpu`, so three models are never resident together (that unload is marked unmeasured on CUDA in the code). Optional `ASR_MAX_SPEAKERS` caps the speaker count. `asr_engine.timed_lines(transcript, turns)` attributes each word by its midpoint (nearest turn when it falls in a gap), numbers speakers in order of first appearance and renders `[MM:SS] Speaker N: …` lines. A line ends on `.` `?` `!`, a pause over `MAX_LINE_GAP` (1.0 s), a change of speaker, or `MAX_LINE_SECONDS` (12 s); a short cue joins the line before it when the speaker is the same, the gap is under `JOIN_GAP` (1.5 s) and that line is under `JOIN_CHARS` (140). Minutes do not roll over into hours (`[75:03]`), so a stamp reads straight off a player. Diarization failing or returning no turns → the lines are still timed, just unlabelled (`[MM:SS] …`), on both engines; a transcript with no word timings comes back as plain text. `asr_engine.untimed(text)` strips the stamps and merges each run of one speaker into a paragraph — that is what the summariser reads (§6.4), because the stamps cost about half as many tokens again. **Punctuation:** the aligner returns bare words; `asr_engine.restore_punctuation` matches them against the text's tokens (`difflib.SequenceMatcher` over alphanumeric lowercase forms) and copies the punctuated token onto every word that matches (`autojunk=False`); it runs on the MLX path and in `align_with_transformers`. The earlier all-or-nothing `len(tokens) == len(words)` check failed on long audio (14 extra aligner words over an 86-minute lecture) and wrote transcripts with no full stops at all — two lectures in the owner's vault had 14,505 and 26,822 words with zero sentence ends. Notes ingested before this fix keep those transcripts until the attachment is redone, since the note holds no punctuated copy to migrate from.
...
The opening marker may carry one optional attribute (`extract_open(src, mode)` writes it; `_OPEN = r'<!-- orb:extract src="([^"]*)"(?: mode="([a-z]+)")? -->'` reads it). There is one mode, `NOTES_MODE = "notes"`:

```
<!-- orb:extract src="…" -->                graphed in full, like the rest of the note
<!-- orb:extract src="…" mode="notes" -->   body = header line, a model-written summary, then the full text under "## Transcript" or "## Full text"
```

A notes block for a recording (a long document has `## Summary` + prose, then `## Full text`):

```
<!-- orb:extract src="attachments/lecture.m4a" mode="notes" -->
[Audio Transcript (lecture.m4a)]:
# <title>
...
`extraction_blocks(content)` returns every block as `{src, key, mode, body}` (`key = attachment_key(src)`, `mode` `""` when absent); `split_notes(body) -> (summary part, full text)` cuts a notes body at the first `## Transcript` / `## Full text` heading line (`_FULL_TEXT_RE`; a body without one is all summary part); `graph_text(content)` is the note with every notes block reduced to its summary part and every other block left whole. See §6.4.
...
`<filename>` is the link text (or URL basename), unescaped. `finish_attachment` finds the header with `_HEADER_RE = r"^\s*(\[.*?\)\]:)[ \t]*"`, which tolerates brackets inside the file name. In a notes block the header line is followed by the summary and the full-text heading rather than by the raw text. `<title>` is the batched LLM title or the filename.
...
Only delimited blocks are touched. A bare `[Image: …]` paragraph the user typed is their text and is never stripped. Notes enriched before delimiters existed get their markers from the one-time vault sweep instead: `vault_sync.migrate_vault_files` (run from `sync_vault_notes`, gated by `<vault>/.orb/migrated-v4`) calls `wrap_legacy_enrichment_blocks(content)`, which wraps each legacy header — matched by `_ENRICHMENT_BLOCK_RE` (`PDF Extraction`, `Image:`, `Audio Transcript`, `Video Audio Transcript`, `Video Visual Analysis`, `Word Extraction`, `Spreadsheet Extraction`, `Unsupported`, each preceded by a blank line) — and the text up to the next header, the next delimited block, or the end of the note in `<!-- orb:extract src="" -->…<!-- /orb:extract -->`. Text already inside a delimited block is left alone, so the wrap is idempotent. From then on those blocks are found, kept or dropped by the same rules as any other. `content_changed` is True when stripping alone changed the body, so a re-ingest of a note whose attachments were removed still rewrites the `.md` without the stale blocks. A block with or without a mode counts as processed (`extraction_srcs` reads the `src` whatever the mode) and is kept while its link remains, so a recording is transcribed and summarised once, not on every re-ingest.

### 6.3 Consumers of the markers

- Extraction LLM: sees the blocks as ordinary note text (the `[Image: <title>]` line is designed to make the image an extractable named entity) — except that a notes block contributes only its summary part: `graph_text` cuts the full text before `extraction_node`, `_write_ontology` and `_update_neighborhoods` see the note (§6.4).
- `summarization_node`: hands every notes block's full text (`split_notes(b["body"])[1]`) to `IngestionWorkflow._index_documents` ([10](10-ingestion-pipeline.md) §6.5).
...
### 6.4 Notes blocks: recordings and long documents

A 400-page PDF or a 90-minute lecture graphed in full costs hours of extraction calls. Ingestion therefore graphs a summary and keeps the full text searchable — automatically; nothing is asked. The rule lives in one function, `finish_attachment(kind, section, filename, llm, set_status) -> (section, mode)`, called by `multimodal_node._run_phase` for every non-image attachment and by the editor's per-attachment job `api/notes._run_attachment_job`:

| Attachment | Block written | Model call |
|---|---|---|
| Recording (`kind` in `("audio", "video_audio")`) — **always**, whatever its length | `mode="notes"`: header line, `llm.summarize_transcript(filename, asr_engine.untimed(text))`, then `## Transcript` + the transcript as extracted (timed lines when available, §5.3) | stage `"Writing notes for <file>"` |
| Any other non-image attachment (PDF, Word, spreadsheet, video visual analysis) with `llm.ingestion_count_tokens(text) > settings.LARGE_ATTACHMENT_TOKENS` | `mode="notes"`: header line, `## Summary`, `llm.summarize_document(filename, text)`, then `## Full text` + the extracted text | stage `"Summarising <file>"` |
| The same at or under the threshold | no mode, the section unchanged | none |
| Image | no mode (`finish_attachment` is not called from the image branch of `_run_phase`, and returns the section unchanged for `kind == "image"`) | none |
| Empty extracted text | section unchanged, no mode | none |

- **Threshold.** `LARGE_ATTACHMENT_TOKENS` defaults to `20000` ingestion-model tokens and is editable in Models → Local runtime as `large_attachment_tokens` (row "Summarize documents over (tokens)", ≥ 1000). It is read at call time. It does not apply to recordings.
- **The summaries.** `summarize_transcript` writes structured notes (`# <title>`, `## Summary`, `### Flow Summary`, topic sections, `### Next Steps` by speaker, `### Decisions Made`); `summarize_document` writes plain prose. Both go piece by piece when the text exceeds about half the ingestion context ([13](13-llm-providers-and-prompting.md) §9.13a–b). A failure propagates like any other attachment error (§7.2).
- **What extraction sees.** `extraction_node`, the `_write_ontology` call in `storage_node` and `_update_neighborhoods` all receive `graph_text(state["content"])`: the summary part of a notes block, never the transcript or full text under it.
- **What search sees.** `summarization_node` passes each notes block's full text to `_index_documents`: one `type='document'` node per attachment, ~1200-character passages in the contexts collection, a fixed one-line description, no LLM call ([10](10-ingestion-pipeline.md) §6.5).
- **Existing notes.** A block written before this behaviour (plain, no mode) stays graphed in full; nothing migrates it. Redoing the attachment from the editor ("process this item") replaces it with a notes block.
...
| `LARGE_ATTACHMENT_TOKENS` | `20000` | `ingestion_agent.finish_attachment` (call time) | A non-image, non-recording attachment whose extracted text has more ingestion-model tokens than this becomes a notes block: summary graphed, full text indexed for search (§6.4). Recordings always do, whatever their size. Local-runtime key `large_attachment_tokens` (Models → Local runtime, "Summarize documents over (tokens)", ≥ 1000), persisted in `runtime_config.json`. |
...
| `ASR_SPEAKERS` / `ASR_DIARIZE_STEP` / `ASR_MAX_SPEAKERS` | `True` / `2.0` / `None` | `_diarizer_ready`, `_speaker_turns` | Speaker labels via pyannote community-1 (GPU when available) + Qwen forced aligner; segmentation step in seconds; cap on speakers (`None` = clustering decides). |
...
9. **Speaker labels need two extra downloads** (aligner + diarizer); the diarizer runs on `mps`/`cuda` when present, else the CPU. Without them the transcript is plain — no timestamps either, since the stamps come from the aligner's word timings — and a single line in the log says why.
...
12. **The size is measured after extraction.** It is only known once the text exists, so a large image-heavy or scanned PDF still pays its per-page vision cost (gotcha 4) before it is summarised; a notes block saves the entity-extraction cost, not the reading cost. The block is kept while its link remains, so both costs are paid once.
13. **One rule, two callers.** `finish_attachment(kind, section, filename, llm, set_status)` decides between a plain block and a notes block; `multimodal_node` and the editor's "process this item" job (`api/notes._run_attachment_job`) both call it, so an audio file or a book-length document gets the same block whichever way its text is extracted. Images never become notes in either path. **Videos differ between the two paths:** a full ingest runs the `video_audio` phase, whose transcript always becomes a notes block, and the `video_visual` phase separately; the editor job classifies the file as `video`, extracts both halves into one section and passes `kind="video"`, which is not a recording kind — so that section becomes notes only over `LARGE_ATTACHMENT_TOKENS`, through `summarize_document` with a `## Full text` heading.
14. **Nothing migrates older blocks.** A transcript or long document ingested before notes blocks existed keeps its plain block and is graphed in full on every re-ingest until the attachment is redone from the editor. The same goes for transcripts written before `restore_punctuation` (§5.3): they have no full stops until redone. A notes block's passages are re-embedded on every re-ingest of the note (the context refresh deletes them first).
...
- `09e6521` (2026-09-15): speaker labels on transcripts (pyannote community-1 + Qwen forced aligner, `Speaker N:` paragraphs — since replaced by timed lines, see `ba995da`); ingestion cancellation.
...
- `bc34e16` (2026-09-21): large attachment prompt — optional `mode` attribute on the marker, `LARGE_ATTACHMENT_TOKENS`, `summarize_document`, `_index_documents`; it parked big attachments as `mode="pending"` and asked graph / summary / index. Removed the same day by `ba995da`.
- `10edef8` (2026-09-21): `restore_punctuation` replaces the all-or-nothing length check; pyannote runs on `resolve_torch_device()` and frees the accelerator afterwards.
- `ba995da` (2026-09-21): notes blocks — the prompt, its three modes (`pending` / `index` / `summary`), `notes.attachment_modes` and `PUT /notes/{id}/attachments/mode` are gone; `finish_attachment`, `mode="notes"`, `split_notes`, `summarize_transcript`, `timed_lines` / `untimed` (replacing `label_speakers`).
