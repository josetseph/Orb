...
- **Speaker labels** (`ASR_SPEAKERS`, default on; needs the `aligner` and `diarizer` snapshots — otherwise "Speaker labels skipped" is logged and the plain transcript is returned): `asr_engine.speaker_turns` runs pyannote `speaker-diarization-community-1` on the accelerator when there is one (`resolve_torch_device`: `mps`, `cuda`, else `cpu`; 40 s on `mps` against 284 s on `cpu` for a 10-minute slice, identical turns) over the already-decoded waveform with segmentation step `ASR_DIARIZE_STEP` (2.0 s; on CPU 1.0 s is 1.8× realtime and 2.0 s 3.4×, while agreeing on 95.7 % of speech; 3.0 s merges speakers). The pipeline is dropped and the `mps`/`cuda` cache emptied afterwards (`release_accelerator_memory`); on the transformers path the speech model and aligner are unloaded first when the device is not `cpu`, so three models are never resident together and optional `ASR_MAX_SPEAKERS`; `label_speakers` attributes each word by its midpoint (nearest turn when it falls in a gap) and renders `Speaker 1: …` paragraphs, one per run of the same speaker, numbered in order of first appearance. Diarization failure → plain transcript. **Punctuation:** the aligner returns bare words; `asr_engine.restore_punctuation` matches them against the text's tokens (`difflib.SequenceMatcher` over alphanumeric lowercase forms) and copies the punctuated token onto every word that matches. The earlier all-or-nothing length check failed on long audio (14 extra aligner words over an 86-minute lecture) and wrote transcripts with no full stops at all; notes ingested before this fix keep those until re-ingested, since the note holds no punctuated copy to migrate from.
...
- **Measure.** After a non-image attachment is extracted, `resolve_large_attachment(...)` (wrapped as `_resolve(section, key, filename)` inside `multimodal_node`) counts `llm.ingestion_count_tokens(section)`. At or under `settings.LARGE_ATTACHMENT_TOKENS` (default `20000`, editable in Models → Local runtime as `large_attachment_tokens`, ≥ 1000) the block is written with no mode and graphed as before. **Images are never measured or parked** (`mode = ""` in the image branch).
...
9. **Speaker labels need two extra downloads** (aligner + diarizer); the diarizer runs on `mps`/`cuda` when present, else the CPU. Without them the transcript is plain and a single line in the log says why.
...
13. **One size rule, two callers.** `resolve_large_attachment(section, key, filename, llm, decisions, set_status)` holds the threshold check and the three modes; `multimodal_node` and the editor's "process this item" job (`api/notes._run_attachment_job`) both call it, so a book-length file is parked with the prompt whichever way its text is extracted. Images are never parked in either path.
