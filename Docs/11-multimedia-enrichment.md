...
**Embedded images** — `extract_docx_images(path, max_images=20) -> list[str]` writes each embedded image part to a temp file and returns the paths (caller deletes them). Parts under 8 KB are skipped as chrome: bullets, rules and spacers describe as noise. `ingestion_agent` calls this **before** the phase loop and appends each image to the `images` list, so they ride the existing image pass rather than forcing a reload in a later phase — previously a diagram inside a Word file was invisible while the same file attached directly got described. Failures are logged and yield `[]`; extraction never blocks the text path.
...
| `PDF_VISUAL_EXTRACTION_ENABLED` | `True` | `_pdf_page_needs_render` | Enables full-page renders for sparse/scanned pages. Embedded-image description is **not** gated by this. |
...
4. **PDF images are described regardless of `PDF_VISUAL_EXTRACTION_ENABLED`**; that flag only controls whole-page renders. A 300-page image-heavy PDF therefore means hundreds of model calls.
...
12. **Embedded `.docx` images are always re-described**: their block `src` is a temp-file path that never matches on the next run, so they are orphaned and redone (a `# ponytail:` comment names the fix — key them by docx url + index).
