...
| `backend/tests/unit/test_large_attachments.py` | The `mode="notes"` marker, `graph_text` (summary only), `split_notes`, `_passages`, and `finish_attachment` for recordings, long / short documents and images, through `multimodal_node` and the editor job | — |
...
5. Phases, each via `_run_phase(stage, model, items, kind)` which sets the stage once and then processes items sequentially through `extract_attachment` (or `describe_image_section` for images), collecting exceptions into `media_errors` instead of aborting; each returned section is placed directly under its link by `place_extraction(content, item["link"], section, mode)` inside `<!-- orb:extract src="<link>" [mode="notes"] -->…<!-- /orb:extract -->` markers. For every non-image attachment `finish_attachment(kind, section, filename, llm, set_status) -> (section, mode)` decides the block: a recording (`audio`, `video_audio`) **always** becomes a notes block — stage `"Writing notes for <filename>"`, `llm.summarize_transcript(filename, asr_engine.untimed(text))`, body = header line, the notes, `## Transcript`, the transcript; any other attachment does only when `llm.ingestion_count_tokens(text) > LARGE_ATTACHMENT_TOKENS` — stage `"Summarising <filename>"`, `llm.summarize_document`, body = header line, `## Summary`, the summary, `## Full text`, the text; anything smaller is written with no mode and no model call. Nothing is asked and nothing is stored per attachment. Images never become notes. Full detail: [11](11-multimedia-enrichment.md) §6.4.
...
8. Returns `content.strip()` — so the extraction sees the enriched text (with each notes block cut down to its summary, §6.3), and `processed_content` in the return value equals the vault body modulo trailing whitespace.
...
- Input text: `extraction_content = graph_text(state["content"])` — the note with every `mode="notes"` block reduced to its summary part (`split_notes(body)[0]`); the transcript or full text under `## Transcript` / `## Full text` never reaches entity extraction. Blocks without a mode are graphed whole. `storage_node` and `summarization_node` pass the same `graph_text(...)` to `_write_ontology` and `_update_neighborhoods`. If `input.title` is set it is prepended as `# {title}\n\n`. (Re-ingested notes always carry their current `note.title`, so in practice the title is present after the first ingest.)
...
- `ingestion_generate_with_meta` / `ingestion_generate` (`services/llm.py`) route to the **ingestion** provider client, which is the chat provider (the instance's `ingestion_provider` constructor argument, else `self.provider`; per-KB services pass the KB's own provider, and there is no settings key), with the model from `get_ingestion_model()` — the KB's `llm_ingestion_model` override, else the model selected for chat. For local there is **no fixed `max_tokens` cap any more**: `LocalLlamaRuntime` sizes the output to `n_ctx − prompt_tokens − safety margin` and raises when the prompt alone (nearly) fills the window ("split the input or raise LLAMA_N_CTX"). Cloud defaults: Anthropic gets `ANTHROPIC_MAX_OUTPUT_TOKENS`; OpenAI/HF unlimited unless passed. Grammar-constrained / structured output is deliberately not used: small GGUFs tend to emit empty nested arrays under schema enforcement.
...
`summarization_node`: skips (`return {}`) when `errors` is set or `extraction` is missing; otherwise stage `"Indexing entity contexts"` / `"Embeddings"`, then `await workflow._update_neighborhoods(extraction.nodes, graph_text(content), note_created_at=state["created_at"], note_id=state["note_id"])`, and — when the note has `mode="notes"` blocks — stage `"Indexing documents for search"` / `None` and `await workflow._index_documents(note_id, documents, created_at)` (below). Any exception → `{"errors": ["Context indexing failed: …"], "status": "FAILED"}`; success → `status="INDEXED"`.
...
**Indexed documents — `_index_documents(note_id, documents, note_created_at)`.** `documents` are the `extraction_blocks(content)` entries with `mode == "notes"`, each with its `body` replaced by the full-text half (`split_notes(b["body"])[1]`) — a recording's transcript or a long document's text ([11](11-multimedia-enrichment.md) §6.4). No LLM call is made. Per document, in one worker thread:
...
| Notes blocks (`mode="notes"`) | The block is kept (not re-extracted, not re-summarised); the context refresh deletes the passages with the note's other contexts and `_index_documents` re-embeds and re-writes them under the same deterministic node id. |
| Plain blocks written before notes blocks existed | Kept and graphed in full; nothing converts them. Redoing the attachment from the editor writes a notes block. |
...
| 5b | `Indexing documents for search` (only with `mode="notes"` blocks) | `summarization_node` → `_index_documents`, `_passages` | Kuzu `document` node + `REFERENCES`; Qdrant `node_cores` (fixed description) and `node_isolated_contexts` (passages, replace-by-parent); Meili doc | inside the same `try` as stage 5 → `Context indexing failed: …` → **failed** |
...
`processed`, `failed`, `processing_stage`, `processing_model`, `title`. There is no body column: the body is the vault file. Full stage vocabulary:
...
| `LARGE_ATTACHMENT_TOKENS` (Models → Local runtime) | `20000` | `ingestion_agent.finish_attachment` | A non-image, non-recording attachment whose extracted text is larger than this (ingestion-model tokens) is summarised for the graph and indexed in full for search; recordings always are ([11](11-multimedia-enrichment.md) §6.4). Min 1000 through the API. |
...
- **`bc34e16`** (2026-09-21): large attachment prompt — `LARGE_ATTACHMENT_TOKENS`, marker `mode` attribute, `graph_text` in front of extraction / `_write_ontology` / `_update_neighborhoods`, `_index_documents` + `_passages`; its `pending` / `index` / `summary` modes, `notes.attachment_modes` and `PUT /notes/{id}/attachments/mode` were removed by `ba995da`.
- **`ba995da`** (2026-09-21): notes blocks — `finish_attachment`, `mode="notes"`, `split_notes`; recordings always, long documents over the threshold, no prompt.
