...
- JSON repair: `llm._clean_json(raw)` unwraps ```` ```json ```` fences, normalises curly quotes, then `json_repair.repair_json` (a hard import). Then `Extraction.model_validate_json(cleaned)`.
...
4. `graph.create_or_update_relationship(source_name, "Indexable", target_name, "Indexable", relationship_type, confidence, strength, relevance, natural_language=rel.natural_language or "", note_id, source_id, target_id)`:
...
14. **`sentiment`, `type_reasoning`, `reasoning` are never persisted** — the prompt no longer asks for the last two; do not look for them in any store.
