...
- **Does not fit** → `_extract_task_split`, three prompts instead of one big one:
  ...
  3. **Contexts** (`ContextPass`) — batched `_CONTEXT_BATCH` entities at a time, and this pass *may* still chunk the text. That is safe here in a way it was not before: the entity list is already fixed, so splitting cannot invent an entity or strand one across two spellings. Descriptions from several pieces are concatenated.
