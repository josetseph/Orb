...
- **Chunked extraction** (`_extract_with_chunking(_llm, extraction_content, logs)` → `(Extraction, chunk_count)`):
  ...
  2. `budget = chunk_token_budget(_llm.ingestion_context_tokens(), overhead, model_name)` from `extraction_chunking.py`: `room = context − overhead − 64`; `fits = room / (1 + 2.5)` (extraction JSON is ~2–3× the input); result `= max(400, min(ceiling, fits))` where `ceiling = extraction_budget.learned_budget(model, 4000)` — the per-model output ceiling learned from truncations (see §11, "Chunk budget is learned"), pinned by `ORB_EXTRACTION_CHUNK_TOKENS` when set. Local context comes from `_default_chat_n_ctx()` (`ORB_LLAMA_N_CTX`); cloud providers report a large window so the learned ceiling binds.
...
| `SEMANTIC_REL` edges | Same `(src, tgt, rel_type)` → `reinforced`: `mention_count += 1`, `last_updated` bumped (no score columns are touched). A differently-worded predicate for the same pair creates an **additional** edge. Edges the new extraction no longer produces remain. |
