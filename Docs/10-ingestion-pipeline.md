...
- **Does not fit** → `_extract_task_split`, three prompts instead of one big one:
  ...
  3. **Contexts** (`ContextPass`) — batched `_CONTEXT_BATCH` (100) entities at a time, and this pass *may* still chunk the text. Cost here multiplies pieces by batches, which is how task-splitting can end up **more expensive than the chunking it replaced**: asking every batch about every piece put a 336k-character note at 74 calls against 22 for plain chunking. `_entities_mentioned_in` restricts each piece to the entities whose names actually occur in it — a piece cannot describe an entity it never mentions — bringing the same note to ~10 calls, or ~30 in the pathological case where every entity appears everywhere. That is safe here in a way it was not before: the entity list is already fixed, so splitting cannot invent an entity or strand one across two spellings. Descriptions from several pieces are concatenated.
