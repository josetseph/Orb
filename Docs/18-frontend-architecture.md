...
| `src/components/graph3d/index.ts` | Barrel for the 3D graph building blocks | re-exports below |
...
| `updateKBLLM(id, {provider?, model?, ingestion_model?})` | `PATCH /kb/{id}/llm` | Empty string / `null` clears a field back to "inherit Settings". Returns the same `KBLLMConfig`. |
...
| `updateLLMSettings(partial)` | `PATCH /settings` | Only changed keys are sent by the settings page. |
...
Fixed `aside` (`left-0 top-0 h-screen w-20 z-40`, black 50 % + blur). `main` has `ml-20` to compensate. Order and icons:
...
| Notes graph | `/notes-graph` | `Link2` | |
| Graph | `/graph-3d` | `Box` | `/graph` redirects here |
| Finance | `/finance` | `Wallet` | |
| Knowledge Bases | `/kb` | `Database` | `title` shows `active: <slug>`; a purple dot badge appears when `currentKB !== "default"` |
| Setup | `/setup` | `Sparkles` | |
| Settings | `/settings` | `Settings` | |

Active state uses **exact** `pathname === item.href` (so `/notes?note=…` still matches `/notes` because `usePathname()` excludes the query, but a hypothetical nested route would not highlight). Bottom of the rail: KB label (first 5 chars + "…" when longer than 6; purple when non-default, else 30 % white) and the `SystemStatusIndicator`. Tooltips for the status light open to the **right** into the main pane because the rail is too narrow for centred tooltips.
...
| `isImageUrl` / `isVideoUrl` / `isAudioUrl` / `isPdfUrl` | Extension regexes (`jpg jpeg png gif webp svg` / `mp4 webm mov mkv m4v` / `m4a mp3 wav ogg aac flac` / `pdf`) applied to the URL-decoded string, tolerant of `?query`. |
...
Mounted once (inside `Sidebar`), keyed on `currentKB`. Polls `api.getMaintenanceStatus(currentKB)` with adaptive `setTimeout` scheduling:
...
| Idle | 30 000 ms |
| Request failed | 30 000 ms (state `failed=true`) |
...
|---|---|---|---|
| `error` | Offline | last request threw | "Could not reach maintenance status. Backend may be restarting." |
| `idle` | Checking… | no payload yet | |
| `ingest` (amber, pulsing) | Ingesting | `ingestion.active > 0` | "N note ingestion(s) running." |
...
| Adaptive polling with visibility pause | `SystemStatusIndicator` (4 s/30 s), settings maintenance (3 s/15 s), chat status (1 s, capped) | The indicator mounts on every page; a fixed fast poll would be a constant background load. |
