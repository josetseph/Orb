...
`build.py dist`, and uploads the bundles as artifacts. Native Python wheels are never
cross-compiled: each platform builds on its own runner.

Version locations to bump together: `desktop/src-tauri/tauri.conf.json` and
...
and notarizes when the variables above are present, using the entitlements in
`desktop/build/entitlements.mac.plist`. Auto-update is not wired: when it is, it uses
