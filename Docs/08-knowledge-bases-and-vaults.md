...
## 19. Addendum: per-KB LLM overrides (uncommitted working-tree change)

The working tree (not yet committed at the time of writing) adds a per-KB chat/ingestion LLM pin. Summary of the contract so other docs stay consistent:

- **Columns** on `knowledge_bases` (raw DDL in `_connect()` + `_ensure_*_columns` ALTERs, mirrored in `models/kb.py`): `llm_provider TEXT`, `llm_model TEXT`, `llm_ingestion_model TEXT`, all nullable; `NULL` = inherit system `Settings`. `_save_row` upserts them. Embed / rerank / multimodal are deliberately **not** per-KB (comment: *"embed dims are shared across every KB's Qdrant collections"*).
- **`KBContext`** gains `llm_provider`, `llm_model`, `llm_ingestion_model`, a cached `_llm` (+ `_llm_built_for` key), `has_llm_override`, the `llm` property (global `llm_service` when no override, else `build_kb_llm_service(provider, model, ingestion_model)` → `LLMService(prov, chat_model=…, ingestion_model=…, ingestion_provider=prov)`, rebuilt when the key changes), and `apply_llm_override(...)` which also drops the cached `retrieval_service` / `ingestion_workflow` / `chat_workflow` so `_ensure_lazy()` rebuilds them with the new `llm=`.
...
- **Frontend**: `frontend/src/app/kb/_components/KBModelPanel.tsx` + new `api.ts`/`types.ts` entries. See [13 LLM providers](13-llm-providers-and-prompting.md) for how `LLMService` consumes the pin.
