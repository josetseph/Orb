...
| 11 | [Multimedia enrichment](11-multimedia-enrichment.md) | Attachment discovery, PDF/image/audio/video/document handlers, vision via the chat model's projector, Qwen3-ASR and Marlin usage, `orb:extract` enrichment block format, notes blocks (recordings and long documents: summary graphed, full text indexed for search), timed speaker transcripts, temp-file rules |
