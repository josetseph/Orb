...
- **Switch script** (`_php_switch_group_script(group_id)`) sets `users.user_group_id = <group>` and saves. It is skipped when `_switched_group_id` already equals the target — commit `f8f527f` ("cache Firefly scope switches") added this because every request previously paid the ~1 s Laravel bootstrap. The cache is **process-local but verified**: when it says the target group is already active, `_firefly_active_group()` does one `GET /api/v1/user-groups` and reads the group flagged `in_use` (Firefly's `UserGroupTransformer` sets it from `users.user_group_id`); if that is a *different* group (e.g. the user switched administrations in Firefly's own web UI at :17412) the cache is dropped and the PHP switch runs again. If the GET fails or no group reports `in_use`, the cache is trusted. Requests still carry `user_group_id=<group>` as a query param, which covers endpoints that honour it.
...
| `_switched_group_id` | `int | None` | Cache of the last group activated via PHP. Cleared by `destroy_kb_administration` when it matches, and by `_activate_scope_locked` when Firefly reports a different `in_use` group. |
...
2. Five `GET /api/v1/accounts` calls with `params={"type": t, "limit": 100, "date": <today>, "user_group_id": gid}` for `t in (asset, cash, expense, revenue, liability)`. Firefly files cash under its own `type=cash` (an asset-like account), so it is queried explicitly; `create_transaction`'s `_match_account(…, {"expense","cash"})` and `summary().asset_balance` therefore see cash accounts.
...
4. `asset_balance = Σ balance` of accounts with `account_type in ("asset", "cash", "liability", "liabilities")` — liabilities are **added**, not subtracted (Firefly reports liability balances as negative numbers when `liability_direction` is debit, so the sum usually nets correctly, but it depends on Firefly's sign convention).
...
- **`_finance_error(exc)`**: `ValueError` → 400, `RuntimeError` (incl. `FireflyHTTPError`) → 502, else 500 — all with `detail: str(exc)`. Applied to **every** `/api/v1/finance/**` route by the `_finance_errors` decorator (`functools.wraps`, so FastAPI still sees the real signature); `HTTPException`s pass through. `GET /finance/workspace` additionally catches inside the service.
...
|---|---|---|
| PHP binary missing (`php/php` absent, or `FIREFLY_RUNTIME_FILE` unset) | first scoped call for a KB without a cached switch | `RuntimeError("Embedded PHP binary not found at …")` → `get_workspace` returns `status: "error"`; every other finance route 502. Fix: let the shell re-provision (`ensurePhpRuntime` re-downloads when the marker mismatches) or delete `firefly/php`. |
...
- **Switch cache is re-verified per scoped call** (one `GET /api/v1/user-groups`, `in_use` flag) — a user switching administrations in Firefly's web UI costs one extra PHP switch on the next call, not a wrong-administration write. If the Firefly version ever drops `in_use`, the cache is simply trusted again.
...
- **Every finance route is wrapped by `_finance_errors`** — a Firefly outage during `refresh()` produces `{"detail": …}` 502s for accounts/transactions (which are required); budgets/categories/etc. still degrade to `[]` silently in the UI.
...
- **Add a new Firefly resource to the backend:** (1) if it has a `user_group_id` column, add the Eloquent model name to the `allowed` set in `_php_model_ids_for_group_script` **and** to the model list in `_php_destroy_group_script`; (2) write `_normalize_<x>`; (3) implement `list_<x>` via `_list_scoped_resources(group_id, model=…, path=…, normalize=…)`, and `create_<x>`/`delete_<x>` as closures passed to `_run_scoped`; (4) add routes in `api_desktop.py` with a Pydantic body and the `@_finance_errors` decorator (the GET too — `test_finance_route_errors.py` enforces it); (5) document in 07.
