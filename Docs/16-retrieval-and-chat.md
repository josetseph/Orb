...
**Indexed documents need no special handling.** A large attachment the user chose to "Index for search" ([10 §6.5](10-ingestion-pipeline.md)) is one `type='document'` node whose text sits as passages in `node_isolated_contexts`, with `name` and `type` in each passage payload. Because context hits are accumulated per name, only the passages that **matched the query** end up in that candidate's `summary`, and so only those reach the reranker and the chat prompt — never the whole document. When the document is reached another way (its name vector in `node_cores`, a Meilisearch match), `_get_node_text` falls back to the node's `description`, which `_index_documents` keeps a fixed one-liner (`"Indexed document (N passages, searchable, not graphed)."`) for exactly that reason: a description built from every passage would pull the whole file into the prompt. The note is cited through the `REFERENCES` edge like any entity's. What an indexed document does not give: its concepts are not entities, so there is no multi-hop expansion through them and nothing in the entity panel.
