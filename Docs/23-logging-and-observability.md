...
| `GET /api/v1/setup/status` | `setup/page.tsx` (polled during downloads), `settings/page.tsx`, `ai-limited-banner.tsx` | `data_dir`, `models_dir`, `paths_json`, `default_vault_path`, `active_vault_path`, `ai_setup_mode`, `ai_configured` (`ai_gate.ai_is_configured()`), `local_models_ready` (`gguf_paths_if_present()`), `multimodal_ready` (all three HF snapshots ready), `needs_model_download` (`mode=="local" and not local_models_ready`), `database_backend`, `llm_provider` | Cheap: filesystem stats only; safe to poll |
